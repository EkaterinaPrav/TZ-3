...
Эта диаграмма описывает процесс обработки заказа системой. Процесс начинается с пользователя, который выбирает ресторан, просматривает меню, добавляет блюда в корзину, а затем оформляет заказ. После этого пользователь выбирает сбособ оплаты, вводит контактные данные и применяет промокод, если он имеется. Затем система проверяет наличие промокода, выбирает способ оплаты и отправляет запрос на заказ в ресторан. Ресторан подтверждает заказ и уведомляет пользователя об успешном размещении. Если оплата проходит успешно, система также уведомляет пользователя об успешной оплате. Затем заказ забирается курьером из ресторана для доставки.
